...
#'
#' @examples
meta.AFT <- function(formula,
                     subset,
                     ldata,
                     B = 500) {
  ## Check format of parameters is correct
  if(class(ldata) != "list")
    stop("ldata should be a list of datasets!") ## FIXME

  ## Need to check factor variables have common level
  ## Across studies
  ## FIXME

  n.study <- length(ldata)
  study <- unlist(lapply(1:n.study,
                         function(i.study)
                           rep(i.study, nrow(ldata[[i.study]]))))
  data.all <- Reduce("rbind", ldata)
  ...
  estimate <- gehan.fit(y = y,
                        delta = delta,
                        matX = matX,
                        matZ = matZ,
                        study = study,
                        missing = missing,
                        beta.ini = rep(0, ncol(matX) + ncol(matZ))) ## FIXME

  # Inference
  ## Create overall studi
  if(n.study == 2)
    # check data are the same
    for(i in 1:n.study) {
      ldata[[i]] <- as.data.frame(l.data[[i]])
      if(i >= 2) if(setequal())
    }
  ldata <- lapply(ldata, as.data.frame)
  if()
    Call <- match.call()
  if(sapply(ldata))
    data <- Reduce("rbind", ldata)
  ind_study <- sapply(ldata, )
  # Extract missing patterns
  indx <- match(c("formula", "study", "data", "subset"),
                names(Call),
                nomatch = 0)
  if (indx[1] == 0)
  ...
  type <- attr(Y, "type")
  if (type != "right")
    stop("Currently only supports right-censored data!")
  data.n <- nrow(Y)
  if (control$timefix)
    Y <- aeqSurv(Y)
